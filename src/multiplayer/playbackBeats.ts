...
const DURATION: Record<PlaybackBeatKind, number> = {
  PLAY: 1150, DRAW: 850, COLOR: 700, SWAP: 1250, ROTATE: 1350,
...
function make(batch: GameEventBatch, event: PendingGameEvent, kind: PlaybackBeatKind, extra: Partial<PlaybackBeat> = {}): PlaybackBeat {
  return { id: `${batch.batchId}:${event.sequence}`, kind, actorPlayerId: event.actorPlayerId, durationMs: DURATION[kind], resultVersion: event.resultVersion, ...extra };
}
...
export function buildPlaybackBeats(batch: GameEventBatch): PlaybackBeat[] {
  ...
  for (const event of batch.events) {
    ...
    switch (event.type) {
      case "PLAY_CARD":
        beats.push(make(batch, event, "PLAY", { card: { color: event.payload.color, type: event.payload.cardType } }));
        break;
      case "CHOSE_COLOR":
        if (previous?.kind === "PLAY" && previous.actorPlayerId === event.payload.playerId && previous.resultVersion === event.resultVersion) previous.chosenColor = event.payload.color;
      ...
      case "CHOSE_SWAP_TARGET":
        ...
        break;
      case "HANDS_SWAPPED":
        if (previous?.kind === "SWAP" && previous.actorPlayerId === event.payload.playerAId) previous.targetPlayerId = event.payload.playerBId;
...
const COLOR_LABEL: Record<CardColor, string> = { RED: "Rot", BLUE: "Blau", GREEN: "Grün", YELLOW: "Gelb", WILD: "WILD" };
const CARD_LABEL: Record<CardType, string> = {
  ...
  SWAP_HAND: "SWAP", SKIP: "SKIP", REVERSE: "REVERSE", DRAW_1: "DRAW 1", DRAW_2: "DRAW 2", WILD: "WILD",
