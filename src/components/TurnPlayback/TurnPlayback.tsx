...
export function TurnPlayback({ beat, players, position, total, canSkip, onSkip }: TurnPlaybackProps) {
  ...
  return (
    <div className={`turn-playback turn-playback--${beat.kind.toLowerCase()}`} data-beat-id={beat.id}>
      <div className="turn-playback__motion" aria-hidden="true">
      ...
      <div className="turn-playback__narrator" role="status" aria-live="polite" aria-atomic="true">
        ...
        {canSkip && <button type="button" onClick={onSkip}>Überspringen</button>}
      </div>
    </div>
  );
}
