...
export function PlayerGame({ session }: PlayerGameProps) {
  ...
  const canUseMainActions = isMyTurn && !busy && !playback.isPlaying && !needsColor && !needsSwapTarget && !needsSkipTarget && !needsExtraDiscard;
  ...
  return (
    <main className="player-game">
      ...
      <div className="player-game__board"><Table publicState={publicState} compact ownPlayerId={session.playerId ?? undefined} playback={playback.activeBeat ? { beat: playback.activeBeat, position: playback.position, total: playback.total, canSkip: playback.canSkip, onSkip: playback.skip } : null} /></div>
