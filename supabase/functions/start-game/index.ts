...
Deno.serve(async(req)=>{ if(req.method==="OPTIONS") return new Response("ok",{headers:corsHeaders()});
  ...
  try { const body=await req.json(); const context=parseMutationContext(body,true); const {deviceId,sessionToken}=body; const admin=supabaseAdmin(); const device=await authenticateDevice(admin,deviceId,sessionToken);
